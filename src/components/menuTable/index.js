...
const MenuTable = (_) => {
    ...
    columns: [
      {
        ...
        field: "title",
        editComponent: (props) => (
          <MenuTextField props={props} label="Título" onChange={handleTitle} />
        ),
      ...
      {
        title: "hora da confirmação",
        field: "hourConfirmReserve",
        editComponent: (props) => (
          <TimeTextField
            props={props}
  ...
  const rowDelete = async (oldData) => {
    ...
    try {
      await api.delete(`/menu`, { data: { id: oldData._id } }).then((resp) => {
        console.log(resp);
        const data = [...state.data];
